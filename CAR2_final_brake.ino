...
float x_ir=0,x_ir_last=0,x_ir_int=0,x=x_ini,x_en=x_ini,x_en_last=x_ini,x_last=x_ini,x_desire=x_ini,x_int=0,
      ...
      v_en=0,v_need=0,v_base=0,y_c2c=y_c2c_ini,
...
//float y=-car_length+20+wheel_to_ir+middle_to_wheel;
...
float stop_time=0;
void enforce_fan(int a){
  /*if(v==0&&brake==0){
    a+=5;
    if((current_time-stop_time)/1000>0.5){
      fanbase=1120;
    }
  }
  else stop_time=current_time;*/
  a=constrain(a,900,1150);
  Serial.print(a);
...
void loop() {
  ...
  if(us_sense||c2c_on<0.5){
    v_need=v_base+PID2(1.6,0,0.5,c2c_error,c2c_error_dev,&c2c_error_int)*float(c2c_on);
  }
  else{
    if(fb)v_need=700;
    else v_need=200;
  ...
  v_update();
  x_map();
  brake=tau_br*(brakebase-PID3(1,0.8,0.5,v_error,-a,&v_error_int))+(1-tau_br)*brake_last;
  if(v<300 && brake<=b_min&&circuit<4){
    enforce_fan(fanbase0+25);
  }
  else enforce_fan(fanbase);
